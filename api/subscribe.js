...
module.exports = async (req, res) => {
  ...
  const AUDIENCE_ID = (process.env.MAILCHIMP_AUDIENCE_ID || process.env.MAILCHIMP_LIST_ID || '').trim();
  ...
  const dc = ((process.env.MAILCHIMP_SERVER_PREFIX || API_KEY.split('-').pop()) || '').trim();
  if (!dc) {
  ...
  try {
    ...
    // Auth and config failures are our problem, not the fan's — keep the detail in the logs.
    if (mc.status === 401 || mc.status === 403 || mc.status === 404) {
      return res.status(500).json({ ok: false, error: 'Signups are temporarily unavailable. Please try again soon.' });
    }
    // Genuine input problems (fake-looking address, compliance state) are safe to relay.
